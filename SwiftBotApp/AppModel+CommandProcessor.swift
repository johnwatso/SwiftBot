    ...
    func makeCommandProcessor() -> CommandProcessor {
        CommandProcessor(
            dependencies: .init(
                ...
                swiftMinerSlashCommand: { [weak self] action, game, userID, channelID in
                    guard let self else {
                        return (ok: false, message: "SwiftMiner integration is unavailable right now.", embed: nil)
                    }
                    return await self.swiftMinerSlashCommand(action: action, game: game, userId: userID, channelId: channelID)
                },
