...
// MARK: - Metadata stubs (legacy web admin)
//
// The web admin's rule editor expects rich metadata on each enum
// (icon symbols, required/provided variables, categories). Those
// properties returned hand-curated data in the old block-builder.
// Now they return blank defaults — the web UI will render an empty
// editor until it's rewritten against Automations.Rule.
...
extension ActionType {
    var symbol: String { "circle" }
    var category: BlockCategory { .actions }
    var requiredVariables: Set<ContextVariable> { [] }
...
// MARK: - RecipeTemplate (legacy stub)
...
struct RecipeTemplate {
    ...
    var symbol: String = ""
    var trigger: TriggerType = .messageCreated
    var actionType: ActionType = .sendMessage
