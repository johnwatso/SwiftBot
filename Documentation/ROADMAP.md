...
- [x] Stopped a synthesizer that changes output format mid-utterance from being merged under the first chunk's format, which resampled the whole render at the wrong ratio and played back stretched and distorted. Mismatched chunks are now reconciled and logged, `copyFrames` bounds-checks the destination, and an Int16 synthesizer no longer merges to silence for want of a matching branch.
...
- [x] Validated live on 2026-08-21 instead of in a disposable channel, the MLS transition path having no offline coverage. Two full sessions completed the handshake through to `transitionActivated`/`mediaReady`, delivering 1144 encrypted frames with zero encrypt failures, zero keepalive failures across 449 attempts, and a clean teardown and re-establish across a rejoin and an app restart.
